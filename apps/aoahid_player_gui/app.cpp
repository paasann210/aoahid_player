...
bool App::animating() const {
    ...
    if (engine_.busy() || recording() || size_task_.valid() || adb_task_.valid())
        return true;
...
void App::poll() {
    ...
    if (ready(size_task_)) {
        const ScreenSize result = size_task_.get();
        if (result.ok) {
            ...
            log_.message(aoap::Severity::warning, "Screen size not detected. " + result.error);
        }
    }
    if (ready(adb_task_)) {
...
void App::connect() {
    ...
    bool stop_adb = stop_adb_;
    if (stop_adb && recording()) {
        stop_adb = false;
        log_.message(aoap::Severity::info,
                     "The adb server is left running because a recording is in progress.");
    }
    engine_.connect(std::move(selection), setup, stop_adb);
}
...
void App::detect_screen_size() {
    if (size_task_.valid())
        return;
    size_task_ = std::async(std::launch::async, [serial = adb_serial(), wake = wake_] {
        ScreenSize result;
        result.ok = aoap::adb_screen_size(serial, result.width, result.height, result.error);
        wake();
        return result;
    });
}
...
void App::draw_header() {
    ...
    float width = ui::pill_width(connection.c_str()) + spacing + ui::pill_width(playback);
    if (recording())
...
void App::draw_devices_card() {
    ...
    if (phase == Phase::refreshing) {
        ...
        if (ui::icon_button("##refresh_devices", ui::Icon::refresh, button, ui::Tone::quiet,
                            "Search for AOA-capable devices"))
            engine_.refresh();
        ImGui::EndDisabled();
    }
    gap(2);
...
void App::draw_touch_settings() {
    ...
    ImGui::SameLine(0, px(6));
    if (size_task_.valid()) {
        ui::spinner(ImGui::GetFrameHeight() * 0.35f, theme::accent);
    } else if (ui::button("Detect")) {
        detect_screen_size();
    }
    ImGui::SetItemTooltip("Read the screen size with adb (wm size). An override size wins.");
...
void App::draw_connect_card() {
    ...
    const Phase phase = engine_.phase();
    ImGui::BeginDisabled(phase != Phase::idle);
    ui::toggle("Stop the adb server first", &stop_adb_);
    ImGui::EndDisabled();
    ImGui::SetItemTooltip("The adb server can hold the phone's USB interface and block the "
                          "accessory handshake.");
    gap(4);
